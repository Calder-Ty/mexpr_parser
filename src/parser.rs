use self::{parse_utils::{ParseResult, ParseError}, identifier::Identifier};
...
mod identifier {
    use super::{parse_utils::{self, ParseResult, ParseError}, Literal};
...
impl<'a> Literal<'a> {
    ...
    fn try_parse_number(text: &'a str) -> ParseResult<Self> {
        let num_start = parse_utils::skip_whitespace(text);
